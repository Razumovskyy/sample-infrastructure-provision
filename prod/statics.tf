...
resource "openstack_blockstorage_volume_v3" "statics_volume" {
  name                 = "volume-for-statics-server"
  size                 = "30"
...
resource "openstack_compute_instance_v2" "statics_server" {
  name              = "test_statics"
  ...
  block_device {
    uuid             = openstack_blockstorage_volume_v3.statics_volume.id
    source_type      = "volume"
